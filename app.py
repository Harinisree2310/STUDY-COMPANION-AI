...
def main():
    ...
    with st.sidebar:
        ...
        mcq_api_key = os.getenv("GROQ_API_KEY_MCQ")
        summary_api_key = os.getenv("GROQ_API_KEY_SUMMARY")
        flashcard_api_key = os.getenv("GROQ_API_KEY_FLASHCARD")
        
        # Optional: Add a check to ensure keys were loaded
        if not all([mcq_api_key, summary_api_key, flashcard_api_key]):
            st.error("API keys not found. Please ensure you have a .env file with the correct key names.")
            st.stop()
